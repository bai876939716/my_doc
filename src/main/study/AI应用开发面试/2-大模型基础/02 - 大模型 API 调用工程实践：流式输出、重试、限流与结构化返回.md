...
上文默认你理解 Token、上下文窗口、Temperature、Top-p 等基础概念。如果还有疑问，建议先看[《万字拆解 LLM 运行机制》](./02%20-%20LLM%20运行机制：Token、上下文窗口与采样参数怎么影响输出.md)和[《大模型提示词工程实践指南》](../agent/05%20-%20大模型提示词工程（Prompt%20Engineering）是什么？提示词技巧有哪些？.md)。
